...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action_add_voucher'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO vouchers (code, discount_amount, min_order_amount) VALUES (?, ?, ?)");
    if (!$stmt) {
        $stmt = $conn->prepare("INSERT INTO vouchers (code, discount_value, min_order) VALUES (?, ?, ?)");
    }
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            $msg = "<div class='alert alert-danger'>Mã voucher đã tồn tại hoặc bị lỗi!</div>";
        }
    }
}
                ...
                <div class="card-body">
                    <form method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="discount_amount" class="form-control" placeholder="VD: 30000" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold">Đơn hàng tối thiểu (VNĐ)</label>
                            <input type="number" name="min_order_amount" class="form-control" placeholder="VD: 150000" value="0">
                        </div>
                        ...
                        <table class="table table-hover align-middle text-center m-0">
                            ...
                            <?php if ($result && $result->num_rows > 0): ?>
                                <?php while ($v = $result->fetch_assoc()): 
                                    ...
                                    $discount = $v['discount_amount'] ?? $v['discount_value'] ?? 0;
                                    $min_ord  = $v['min_order_amount'] ?? $v['min_order'] ?? 0;
                                ?>
                                <tr>
                                    <td>#<?php echo $v['id']; ?></td>
